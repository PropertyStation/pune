...
const PropertyCard = ({ carddata,form }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [formData,setFormData] = useState(null);
  const handleOpenForm=()=>{setFormData(true);}
  const handleCloseForm=()=>{setFormData(false);}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-10 mt-10">
        {currentProperties.map((property, index) => (
          <div key={index} className="w-84 bg-gray-100 shadow-lg rounded-2xl overflow-hidden border border-gray-200 p-4" onClick={()=>window.open(property.redirectUrl,"_blank")}>
            ...
            <button className="text-[14px] font-normal w-full bg-gray-600 hover:bg-navy-700 text-white py-2 rounded-lg mt-4" onClick={handleOpenForm}>
              Enquire Now
            </button>
          </div>
        ))}
