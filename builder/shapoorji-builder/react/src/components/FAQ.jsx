            ...
            <div className='w-full max-w-2xl space-y-2 mt-10' >
                {faqData.map((item, index) => (
                    <div key={index} className='shadow-xl bg-white rounded-md overflow-hidden transition-all duration-300'>
                        <button onClick={() => (toggleFaq(index))} className="w-full flex justify-between items-center p-4 text-black font-medium text-lg bg-white border-b hover:bg-gray-50 transition">{item.question}                    <FaChevronDown className={`transform transition-transform duration-300 ${openIndex === index ? "rotate-180" : ""}`} />
                        </button>
